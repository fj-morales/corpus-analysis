...
from TapConnectorService import TapAWA as tpAwa
...
class TextPreprocessorTool:
    ...
    def textProcess_AWA3(self,readFromFile = False):
        #all sentences
        #sentence_query = ("SELECT sentence_id,sentence_detail FROM sentence ORDER BY sentence_id ASC")
        #all sentences with specific annotation
        #sentence_query = ("SELECT sentence_id,sentence_detail FROM sentence WHERE sentence_id IN(SELECT DISTINCT sentence_id from sentence_annotation where annotation_id=7) ORDER BY sentence_id ASC")
        #elsvier sentences
        sentence_query = ("SELECT sentence_id,sentence_detail FROM sentence WHERE corpus_id="+ str(self.OASTMCorpusID) + " ORDER BY sentence_id")
        #bawe sentences
        #sentence_query = ("SELECT sentence_id,sentence_detail FROM sentence WHERE corpus_id="+ str(self.BAWECorpusId) + " ORDER BY sentence_id ASC")
        #pmc sentences
        #sentence_query = ("SELECT sentence_id,sentence_detail FROM sentence WHERE corpus_id="+ str(self.PMCCorpusId) + " ORDER BY sentence_id ASC")
        ...
        dicAnnotation = {}
        dbCursor.execute("SELECT annotation_label,annotation_id FROM ANNOTATION WHERE tool_id=3")
        for (annotation_label,annotation_id) in dbCursor:
            dicAnnotation.update({annotation_label:annotation_id})
        ...
        if(readFromFile==False):
            ...
            print("Get data from error file")
            errorFile = open("ErrorFile.txt",'r')
            for id in errorFile.readlines():
                dbCursor.execute("SELECT sentence_id,sentence_detail FROM sentence WHERE sentence_id=" + str(id))
                for (sentence_id,sentence_detail) in dbCursor:
                    dicSentence.update({sentence_id:sentence_detail})
        ...
        print("Sentences populated: ", len(dicSentence))
        annotationScheme = []
        for sentence in dicSentence.keys():    
            responseTapAwa = tpAwaObject.getAnnotationSchemeFromTAP(dicSentence.get(sentence))
            if(len(responseTapAwa)==0):
                continue
            
            try:
                for analyticScheme in responseTapAwa.get('data').get('moves').get('analytics'):
                    annotationScheme = analyticScheme
            except:
                errorFile.write(str(sentence) + "\n")
                continue
            
            if(len(annotationScheme)==0):
                continue
            
            #main categories
            if(("vis" in annotationScheme) and ("contribution" in annotationScheme)):
                sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation["AWA3_Important&Summary"])
                dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                #print("AWA3_Important&Summary")
            elif("vis" in annotationScheme):
                sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation["AWA3_Important"])
                dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                #print("AWA3_Important")
            elif("contribution" in annotationScheme):
                sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation["AWA3_Summary"])
                dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                #print("AWA3_Summary")
            
            #sub categories
            for subCategory in annotationScheme:
                if(subCategory=='contrast'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Contrast'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Contrast")
                elif(subCategory=='emph'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Emphasis'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Emphasis")
                elif(subCategory=='attitude'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Position'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Position")
                elif(subCategory=='novstat'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Novelty'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Novelty")
                elif(subCategory=='nostat'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Question'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Question")
                elif(subCategory=='grow'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Trend'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Trend")
                elif(subCategory=='old'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Background'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Background")
                elif(subCategory=='surprise'):
                    sentenceAnnotation_data = (sentence,3,process_date,dicAnnotation['AWA3_Surprise'])
                    dbCursor.execute(sentenceAnnotation_query,sentenceAnnotation_data)
                    #print("AWA3_Surprise")
                else:
                    continue
        ...
        dbCursor.close()
        dbObject.closeDb(conn)
        print("textProcess_AWA3 complete")