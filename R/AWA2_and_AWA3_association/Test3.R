...
annotationList <- c("Important","Summary","Important&Summary","Background","Contrast","Emphasis","Novelty","Position","Question","Surprise","Trend")
...
for(sentence in corpusSentences$sentence_id){
  for(annotationLabel in annotationList){
    ...
    if(nrow(resultAWA2)>0){
      ...
      sqlStatementAWA3 <- paste0("select an.annotation_label as 'AWA3' from sentence_annotation sa, annotation an where sa.annotation_id=an.annotation_id and an.annotation_label in ('AWA3_Important','AWA3_Summary','AWA3_Important&Summary','AWA3_Background','AWA3_Contrast','AWA3_Emphasis','AWA3_Novelty','AWA3_Position','AWA3_Question','AWA3_Surprise','AWA3_Trend') and sa.sentence_id=",sentence)
...
summaryTable <- rbind("Summary" =summaryTable[4,-1],
                      "Contrast"=summaryTable[3,-1],
                      "Important"=summaryTable[5,-1],
                      "Important&Summary"=summaryTable[2,-1],
                      "Background"=summaryTable[8,-1],
                      "Emphasis"=summaryTable[6,-1],
                      "Position"=summaryTable[11,-1],
                      "Novelty"=summaryTable[7,-1],
                      "Question"=summaryTable[9,-1],
                      "Trend"=summaryTable[10,-1],
                      "Surprise"=summaryTable[12,-1])
...
residualValues <- round(chisquare$residuals, 3)
